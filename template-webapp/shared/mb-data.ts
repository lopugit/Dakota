// Macrobiotica seed data — ported nearly as-is from
// design_handoff_macrobiotica_app/design_files/mb-data.js (ids kept stable).
import type {
  Food, CookMethod, Meal, Ailment, Practice, Course, ArticleBlock, Post,
  Friend, ElementRow, ClockRow, Animal, GlossaryRow, Zodiac, UserLog,
} from './types';

export const CATS: string[] = ['Grains', 'Vegetables', 'Beans', 'Sea vegetables', 'Fermented', 'Fruit', 'Pantry'];
...
export const AILMENT_SYSTEMS: string[] = ['All', 'Digestion', 'Energy', 'Sleep', 'Circulation', 'Skin', 'Mind', 'Cycles'];
...
export const ZODIAC_ELEMENTS: string[] = ['Wood', 'Fire', 'Earth', 'Metal', 'Water'];
...
export function zodiacFor(year: string | number): Zodiac | null {
  const y = Number(year);
...
export function seedLog(now: Date): UserLog {
  const log: UserLog = {};
  ...
  for (let i = 24; i >= 0; i--) {
    ...
    const meals: { id: string; t: string }[] = [];
    ...
    const checkins: { t: string; v: number; note: string }[] = [];
