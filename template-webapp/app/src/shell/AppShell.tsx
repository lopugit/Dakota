...
export function AppShell() {
  ...
  // Restore the last active tab once per load (prototype's mb-tab behavior);
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem('mb-tab');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('mb-tab', activeTab);
    } catch {
      /* private mode */
    }
  }, [activeTab]);
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', minHeight: '100vh' }}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          {!isDesktop && (
            ...
            >
              <span className="mb-wordmark" style={{ fontSize: 20, color: 'var(--text-primary)' }}>
                Macrobiotica
              </span>
              <ThemeToggle />
            </header>
